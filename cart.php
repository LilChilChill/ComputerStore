								...
								<span>
									...
									if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['apply_coupon'])) {
										...
										if ($discount_percent > 0) {
											$total_after_discount = $gtotal - ($gtotal * $discount_percent / 100);
											Session::set('total_after_discount', $total_after_discount);
											echo $fm->format_currency($total_after_discount) . " VNĐ";
										} else {
											// Kiểm tra nếu mã giảm giá không tồn tại hoặc hết hạn
											if ($discount_percent === false) {
												echo "Mã giảm giá không tồn tại hoặc đã hết hạn";
											} else {
										...
										// Nếu không nhập mã giảm giá, hiển thị tổng tiền bình thường
